...
Чат реализован через Socket.IO (WebSocket) и поддерживает сохранение истории сообщений.

- **WebSocket namespace:** корневой `/` (chat)
- **Основные события Socket.IO:**
  - `chat:rooms` — список доступных комнат (отправляется при подключении)
  - `chat:join` — присоединение к комнате; сервер добавляет сокет в комнату и отправляет `chat:history`
  - `chat:history` — история комнаты (последние 100 сообщений), отправляется только текущему сокету
  - `chat:message` — отправка сообщения в комнату; сервер сохраняет сообщение и рассылает всем в комнате через `io.to(room).emit("message", ...)`
  - `message` — общий формат приходящих сообщений (используется для бот-уведомлений и обычных сообщений)

**Поведение:**

- Все сообщения сохраняются в MongoDB (модель: `src/modules/chat/models/chat-message.model.ts`).
- На сервере реализована логика, которая гарантирует, что в БД хранится не более **100 последних сообщений** для каждой комнаты — старые автоматически удаляются (утилиты: `src/modules/chat/chat.utils.ts`).
- При повторном заходе в комнату пользователь получает историю (`chat:history`), клиент предотвращает дубли при подгрузке.
- Бот-сообщения (join/leave) тоже сохраняются.

**REST:**

- `GET /api/chat/:roomId/history` — возвращает историю (последние 100 сообщений) для комнаты `:roomId` (требует авторизации).
  - Ответ: `{ roomId, messages: [ { _id, roomId, userId?, username, text, createdAt } ] }`

Рекомендуется: подключать историю при присоединении к комнате и показывать старые сообщения перед новыми.
