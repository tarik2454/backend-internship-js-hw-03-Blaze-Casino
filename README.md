...
Чат реализован через Socket.IO (WebSocket) и поддерживает сохранение истории сообщений в MongoDB.
...
4. Автоматически удаляет старые сообщения (остается только последние 100)
...
- `"Room {roomId} does not exist"` - комната не существует
...
- Все эндпоинты `/api/chat/*`
- `POST /api/auth/logout` - Выход из системы

**Примечание:** WebSocket подключение для чата также требует авторизации через JWT токен в `socket.handshake.auth.token`.
